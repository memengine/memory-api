...
class ConflictResolver:
    ...
    def _build_merged_memory(
        *,
        ...
        return ExtractedMemory(
            ...
            validated_evidence=dict(getattr(new_memory, "validated_evidence", {}) or {}),
        )
    ...
    def _store_new_memory(
        ...
        *,
        ...
        memory_provenance = self._provenance_for_extracted_memory(
            extracted_memory,
    ...
    def _provenance_for_extracted_memory(
        self,
        extracted_memory: ExtractedMemory,
        *,
        ...
        provenance = dict(self.provenance_snapshot or {})
        validated_evidence = dict(getattr(extracted_memory, "validated_evidence", {}) or {})
        if validated_evidence:
            # These identifiers are generated by the write path, after the
            # evidence policy has accepted the cited turn indexes. They are not
            # model- or caller-controlled values.
            validated_evidence["memory_id"] = str(memory_id)
            validated_evidence["source_conversation_id"] = str(source_conversation_id)
            provenance["extraction_evidence"] = validated_evidence
        ...
        *,
        ...
        try:
            ClaimLedgerService(self.session).record_memory(
                ...
                tenant_id=tenant_id,
                proxy_user_id=proxy_user_id,
                provenance=dict(memory.metadata_json or {}).get("provenance"),
                resolution=resolution,
                decision_evidence=decision_evidence,
    ...
    def _serialize_extracted_memory(memory: ExtractedMemory) -> dict[str, Any]:
        return {
            ...
            "validated_evidence": dict(getattr(memory, "validated_evidence", {}) or {}),
        }
