...
from benchmarks.internal.temporal_cases import load_temporal_cases


def _failure_kind(output: str, code: int) -> str | None:
    if code == 0:
        return None
    markers = ("permissionerror", "connection refused", "keyerror: 'database_url'", "not found", "no tests ran")
...
def main() -> None:
    ...
    rows = []
    for case in load_temporal_cases():
        ...
        rows.append({"id": case.id, "area": case.area, "level": case.level, "test_node": case.test_node,
                     "validates": list(case.validates), "passed": result.returncode == 0,
    ...
    summary = {"scenario_count": len(rows), "evaluable_product_scenarios": len(product),
               ...
               "end_to_end_lifecycle_success": sum(row["passed"] for row in product) / len(product),
    ...
    payload = {"benchmark": "temporal-memory-correctness-development-v1", "captured_at": datetime.now(UTC).isoformat(),
               "holdout_used": False, "production_behavior_changed": False, "summary": summary, "cases": rows}
